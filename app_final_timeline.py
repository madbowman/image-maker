...
class ComfyUIBatchGenerator:
    ...
    def detect_characters(self, prompt, character_names):
        ...
        for char_name in character_names:
            ...
            if first_name.lower() in prompt_lower:
                mentioned.append(char_name)
        return mentioned
    
    def generate_single(self, scene, model_name, scene_number):
        try:
...
def create_ui():
    ...
    """) as app:
            ...
            with gr.Tab("Timeline"):
                with gr.Row():
                    with gr.Column(scale=1):
                        ...
                        hidden_scene_num = gr.Number(value=0, visible=False)
                        hidden_prompt = gr.Textbox(value="", visible=False)
                        hidden_regen_btn = gr.Button("Hidden Regenerate", visible=False)
        ...
        def build_timeline_html(scenes, generated_images):
            ...
            # Add JavaScript for direct regeneration
            html += """
            </div>
            <script>
            function updateAndRegenerate(sceneNum) {
                const textarea = document.getElementById('scene_prompt_' + sceneNum);
                if (!textarea) {
                    alert('Error: Could not find prompt textarea for scene ' + sceneNum);
                    return;
                }
                ...
                // Find and update the hidden inputs
                const numberInputs = document.querySelectorAll('input[type="number"]');
                const textInputs = document.querySelectorAll('input[type="text"], textarea');
                const buttons = document.querySelectorAll('button');
                
                // Set the scene number in the hidden number input
                for (let input of numberInputs) {
                    if (input.style.display === 'none' || input.parentElement.style.display === 'none') {
                        input.value = sceneNum;
                        input.dispatchEvent(new Event('input', { bubbles: true }));
                        break;
                    }
                }
                
                // Set the prompt in the hidden text input  
                ...
                // Click the hidden regenerate button
                setTimeout(() => {
                    for (let btn of buttons) {
                        if ((btn.style.display === 'none' || btn.parentElement.style.display === 'none') &&
                            btn.textContent.includes('Hidden Regenerate')) {
                            btn.click();
                            break;
                        }
                    }
                    ...
                    setTimeout(() => {
                        button.style.background = originalColor || '#2196F3';
            ...
            <div style='display: flex; gap: 20px; margin-bottom: 25px; padding: 15px; 
                ...
                <div style='flex: 2;'>
                    ...
                    <div style='margin-bottom: 10px;'>
                        <textarea id='scene_prompt_{scene_num}' 
                                  style='width: 100%; padding: 10px; border: 1px solid #ddd; 
                                         border-radius: 4px; font-size: 13px; line-height: 1.6; 
                                         resize: vertical; background: white; min-height: 120px;
                                         color: #000;'
                                  rows='6'>{scene['prompt']}</textarea>
                    </div>
                    
                    <div style='display: flex; gap: 8px; align-items: center; margin-bottom: 8px;'>
                        <button onclick='updateAndRegenerate({scene_num})' 
